...
Describe 'Unit Tests' -Tag 'UnitTests_UT' {
  ...
  BeforeAll {
    ...
    $Mock1ObjFwResource = [PSCustomObject]@{
      FwResource = @(
        [PSCustomObject]@{
          Id     = 1
          String = 'String1'
  ...
  Context 'ParameterSetName [All]' -Tag 'UT_PSN_All' {
    ...
    It 'Assert <Mock1> is called exactly <Mock1ObjFwResource.Count> time' {
      $params = @{
        CommandName = $Mock1
        Times       = $Mock1ObjFwResource.Count
        Exactly     = $true
        Scope       = 'Context'
      }
    ...
    It '$result.Count | Should -BeExactly <Mock1ObjFwResource.FwResource.Count>' {
      $result.Count | Should -BeExactly $Mock1ObjFwResource.FwResource.Count
    }
